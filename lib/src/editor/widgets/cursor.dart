...
class CursorCont extends ChangeNotifier {
  ...
  void _onColorTick() {
    color.value = _style.color
        .withAlpha(200)
        .withValues(alpha: _blinkOpacityController.value);
...
class CursorPainter {
  ...
  void paint(Canvas canvas, Offset offset, TextPosition position,
    ...
    final node =
        controller.document.queryChild(controller.selection.start).node;

    if (node != null && node is Embeddable) {
      caretRect =
          Rect.fromLTWH(caretRect.left, 0, caretRect.width, caretRect.height);
    }
